    ...
    <div class="login-box">
      <div class="login-header">
        <span :class="{'active': isLogin}">
          <router-link to="/login" style="color: #9E9B96;">{{ $t('register.login') }}</router-link>
        </span>
        <span>·</span>
        <span :class="{'active': !isLogin}">{{ $t('register.register') }}</span>
        <div class="register-type">
          <span :class="{'phone-active': isPhone}" @click="changeType(true)">{{ $t('register.mobile') }}</span>
          <span :class="{'phone-active': !isPhone}" @click="changeType(false)">{{ $t('register.email') }}</span>
        </div>
      </div>
      ...
      <div class="login-body">
        ...
        <div class="flex-box align-items-center custom-input-wrap">
          <Icon type="ios-locked" size="18"></Icon><input  @keyup.enter="register" class="custom-input flex-1" v-model="postData.againPassword" type="password" :placeholder="$t('register.password2')">
        </div>
      </div>

      <div class="login">
        <Button type="warning" long @click.native="register">{{ $t('register.register') }}</Button>
        <Checkbox v-model="agree">{{ $t('register.agree') }}</Checkbox>
      </div>

      <div class="split">
        <div class="line"></div>
        <div class="split-text">{{ $t('register.socialAccount') }}</div>
        <div class="line"></div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      register() {
        ...
        if(this.isPhone) {
          // 手机注册
          this.$http.post('/api/register/mobile',{
            email: this.postData.userName,
...
//              this.$store.commit('changeLoginState',true);
          ...
          // 邮箱注册
          this.$http.post('/api/register/email',{
            email: this.postData.userName,
            password: this.postData.password,
            verifyCode: this.postData.verifyCode
          }).then((body) => {
